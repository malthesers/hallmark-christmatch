<template>
  ...
  <main class="text-white min-h-dvh grid place-content-center p-4 pt-[98px] xs:pt-[60px]">
    <section class="flex flex-col gap-4">
      <div :class="{ 'pointer-events-none': paused }" class="grid grid-cols-5 gap-2 sm:gap-4">
        <MovieCard
          v-for="(movie, index) in pairedMovies"
          :key="index"
          :movie="movie"
  ...
  <VictoryModal @closeModal="showVictoryModal = false" :showVictoryModal="showVictoryModal" :guesses="guesses" :record="record" />
</template>
...
function startGame(delay: number = 0): void {
  ...
  setTimeout(() => {
    ...
    pairedMovies.value = shuffle(
      shuffle(movies.value)
        .slice(0, gameSize.value)
